...
import time
from env_opnet_0828 import Env_team#
import copy
...
def return_DQN(name_,fea_num):
    with tf.variable_scope(name_):
        dueling_DQN = DuelingDQN(
            n_actions=ACTION_SPACE, n_features=fea_num, memory_size=MEMORY_SIZE,
        e_greedy_increment=0.001, sess=sess, dueling=True, output_graph=True)
    return dueling_DQN
dueling_DQN_40={}
for i in range(num_agent-4):
    temp_dueling_DQN=return_DQN('dueling'+str(i),5)
    dueling_DQN_40[str(i)]=temp_dueling_DQN
for i in range(num_agent-4,num_agent):
    temp_dueling_DQN=return_DQN('dueling'+str(i),4)
    dueling_DQN_40[str(i)]=temp_dueling_DQN
...
def train(RL,saver,saved_net,RL_greedy):
    ...
    for episode in range(EPISODE):
        ...
        env.reset()
            ...
            #saver.save(sess, saved_net + 'network' + '-dqn', global_step=buffer_count)
...
if __name__ == '__main__':
    ...
    env.get_time_date()
    for i_ in range(1):
        path_name = "saved_networks_opnet"
        ...
        train(dueling_DQN_40,saver,path_name+'/',0.1 )
