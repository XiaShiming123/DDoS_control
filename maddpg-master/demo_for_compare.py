...
def train(arglist):
    with U.single_threaded_session():
        ...
        plt.figure()
        plt.plot(rs_train)
        plt.savefig("MADDPG-train-"+str(re_time)+"_.png")
        np.save("MADDPG_rs_train_", rs_train)
        rs_test = []
        crash_time=0
        action_all=[]
        for k in range(10):
            ...
            for e_i in range(8000, 8502):
                ...
                steps += 1
                if e_i % 100 == 0:
                    print("Test, {}, {}, reward:{}".format(k, e_i, epi_r))
                    print(env.server.is_being_attack,action_n[0:10])
                episode_step += 1
    plt.figure()
    plt.plot(rs_test)
    plt.savefig("MADDPG-test-"+str(re_time)+"_.png")
    np.save("MADDPG_rs_test_",rs_test)
    print(np.mean(rs_test),crash_time)
    np.save("action_all"+str(re_time),action_all)
